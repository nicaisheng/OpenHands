...
class ActionExecutor:
    ...
    It is responsible for executing actions received from OpenHands backend and producing observations.
    Now connects to ActionExecutionClient via WebSocket.
    """
    ...
    def __init__(
        ...
        websocket_url: str = 'ws://0.0.0.0:3001/ws',
    ) -> None:
    ...
    async def ainit(self):
        ...
        logger.debug(f'Starting WebSocket client connection to {self.websocket_url}')
        self.websocket_task = asyncio.create_task(self._websocket_client_handler())
    ...
    async def _websocket_client_handler(self):
        """Connect to ActionExecutionClient WebSocket server and handle messages."""
        retry_count = 0
        max_retries = 1000
        retry_delay = 2
                    ...
                    async for message in websocket:
                        try:
                            ...
                            if data.get('type') == 'action':
                                # Handle action request from ActionExecutionClient
